...
class UserBloc extends BlocBase {
  ...
  void _addUsersListener() {
    _firestore.collection("loja_virtual_users").snapshots().listen(
        (snapshot){
  ...
  void _subscribeToOrders(String uid){
    _users[uid]["subscription"] = _firestore.collection("loja_virtual_users")
        .document(uid)
        .collection("orders")
        .snapshots().listen(
        (orders) async {
          ...
          for(DocumentSnapshot d in orders.documents){
            DocumentSnapshot order = await _firestore.collection("loja_virtual_orders")
